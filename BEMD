...
function Results = BEMD(u,v,param,varargin) 
...
e1 = sqrt( (minima_pos_x(TRI_min(:,2))- ...
    minima_pos_x(TRI_min(:,1))).^2 + ...
    (minima_pos_y(TRI_min(:,2))- minima_pos_y(TRI_min(:,1))).^2 );
...
e = [emn1 ,emn2, emn3]; 
%Smaller edge for each vertex in each triangle (since one vertex is associated with two edges in a triangle)

%Making sure that the smallest edge associated with the 
% each vertex is stored
%correctly
for i=1:length(emn1)
for j=1:3
...
%Order statistics filtering to determine maximum and
% minmum envelopes
Env.u.max = ordfilt2(H.u ,w_sz.^2, true(w_sz),'symmetric'); 
%Max envelope u
Env.u.min = ordfilt2(H.u ,1, true(w_sz),'symmetric');       
%Min envelope u

Env.v.max = ordfilt2(H.v ,w_sz.^2, true(w_sz),'symmetric'); 
%Max envelope v
Env.v.min = ordfilt2(H.v ,1, true(w_sz),'symmetric');       
%Min envelope v            
end
...
% Purpose: 
% To calculate the index of orthogonality of a 
% decomposition and its mean
% squared error
...
titleString = 'u';
set(colorTitleHandle ,'String',titleString,'Interpreter',...
    'latex','FontSize',14);
