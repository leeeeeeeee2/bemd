...
function Results = EMD2D2V(u,param,varargin) 
...
mse_u = immse(H1.u,H.u);    
if (mse_u<param.tol && mse_u<param.tol && sift_cnt(imf)~=1)
sift_stop = 1;
end
...
[Results.IO.u,Results.Error.u] = ...
    Orth_index(u,IMF.u,Residue.u);
...
mask = ones(3); 
mask(5) = 0; 
%Window size for the extrema detection fixed at 3x3 (Bhuiyan et.al)
...
emn1 = min([e2, e1],[],2); 
%Comparing edges 2 and 1 (vertex 1)
emn2 = min([e3, e1],[],2); 
%Comparing edges 3 and 1 (vertex 2)
emn3 = min([e3, e2],[],2); 
%Comparing edges 3 and 2 (vertex 3)
...
function Plot_results(u,~,Results,param)
